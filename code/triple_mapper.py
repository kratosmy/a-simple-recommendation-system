...
frequent3 = {}  # this is for frequent item-set size 3
# find frequent item-set size 3
for k1, k2, k3 in itertools.combinations(frequent2_dic, 3):
    if len(list(set(frequent2_dic[k1]) & set(frequent2_dic[k2]) & set(frequent2_dic[k3]))) >= support:  # check intersection length
        frequent3[k1 + ',' + k2 + ',' + k3] = len(list(set(frequent2_dic[k1]) & set(frequent2_dic[k2]) & set(frequent2_dic[k3])))

for key, values in frequent3.items():
